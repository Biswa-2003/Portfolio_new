...
export interface SphereImageGridProps {
  ...
  className?: string;
  onHoverItem?: (item: ImageData | null) => void;
}
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const handleTouchMove = useCallback((e: TouchEvent) => {
    ...
    // Only prevent default if horizontal drag is dominant (rotating sphere)
  ...
  useEffect(() => {
    ...
    // Touch events - only active when user is dragging to prevent global touch interception
    if (isDragging) {
  ...
  }, [isMounted, isDragging, handleMouseMove, handleMouseUp, handleTouchMove, handleTouchEnd]);
      ...
      >
        <div
          ...
          style={{
            ...
            overflow: 'hidden',
            boxShadow: isHovered
              ? '0 0 25px rgba(56, 189, 248, 0.9), inset 0 0 10px rgba(56, 189, 248, 0.5)'
        ...
        >
          <img
            ...
            style={{
              ...
              padding: image.src.endsWith('.svg') ? '6px' : '0',
            }}
            draggable={false}
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-zinc-950 rounded-xl max-w-md w-full overflow-hidden border border-zinc-800"
        ...
        >
          ...
          >
            <img
              ...
              style={{
                width: '100%',
                height: '100%',
                objectFit: selectedImage.src.endsWith('.svg') ? 'contain' : 'cover',
                padding: selectedImage.src.endsWith('.svg') ? '24px' : '0',
              }}
            />
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 right-2 w-8 h-8 bg-black/60 rounded-full text-white flex items-center justify-center hover:bg-black/90 transition-all cursor-pointer border border-white/20"
              style={{
                position: 'absolute',
                top: '12px',
                right: '12px',
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6" style={{ padding: '24px' }}>
              {selectedImage.title && (
                <h3 className="text-xl font-bold mb-2 text-white" style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '8px', color: '#ffffff' }}>
                  {selectedImage.title}
                </h3>
              )}
              {selectedImage.description && (
                <p className="text-gray-400" style={{ fontSize: '0.92rem', color: '#a1a1aa', lineHeight: 1.6, margin: 0 }}>
                  {selectedImage.description}
                </p>
              )}
            </div>
