...
export async function POST(req) {
  try {
    ...
    const { name, email, message, purpose, channel, hp, tts } = await req.json();
    const selectedChannel = channel || purpose || 'General Inquiry';
    ...
    const html = `
      <div style="font-family:Inter,Arial,sans-serif;line-height:1.6">
        <h2>New Portfolio Transmission (Secure Connection Portal)</h2>
        <p><strong>Registry Identifier:</strong> ${escapeHTML(name)}</p>
        <p><strong>Communication Protocol (Email):</strong> ${escapeHTML(email)}</p>
        <p><strong>Channel / Purpose:</strong> ${escapeHTML(selectedChannel)}</p>
        <p><strong>Encrypted Payload:</strong></p>
        <pre style="white-space:pre-wrap;margin:0;background:#f4f4f5;padding:12px;border-radius:8px">${escapeHTML(message)}</pre>
      </div>
    `;
    ...
    await transporter.sendMail({
      ...
      subject: `[Portfolio Connection] ${escapeHTML(selectedChannel)} - ${escapeHTML(name)}`,
      html,
      replyTo: email,
    });
